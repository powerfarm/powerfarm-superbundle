...
check('Process and JS pin the same ExecutionSlice version', processSlice.includes(`EXECUTION_SLICE_CONTRACT_VERSION = "${EXECUTION_SLICE_CONTRACT_VERSION}"`));
check('ADK re-exports the Process-owned ExecutionSlice contract', adkSlice.includes('from powerfarm.execution_slice import *'));
check('MAF validates and derives refs from the Process-owned ExecutionSlice contract', mafController.includes('validate_execution_slice') && mafController.includes('execution_refs_from_slice'));
